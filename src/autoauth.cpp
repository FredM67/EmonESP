...
void auth_request() {
  ...
  if (result != F("request registered")) {
    ...
    if (stringpart == 2) {
      ...
      DEBUG.println(F("MQTT Settings:"));
      DEBUG.println(result);
    }

    if (stringpart == 3) {
      mqtt_auth_transfer_flag = 2;
      config_save_mqtt(true, mqtt_server.c_str(), mqtt_portnum.toInt(), mqtt_basetopic, "", mqtt_username,
                       mqtt_password);
      DEBUG.println(F("MQTT Settings:"));
      DEBUG.println(result);
    }
...
void auth_setup() { Udp.begin(localUdpPort); }
