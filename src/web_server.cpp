...
// -------------------------------------------------------------------
bool requestPreProcess(AsyncWebServerRequest *request, AsyncResponseStream *&response,
                       const __FlashStringHelper *contentType = CONTENT_TYPE_JSON) {
  dumpRequest(request);
  ...
  response->addHeader(F("Cache-Control"),
                      F("no-cache, private, no-store, must-revalidate, max-stale=0, post-check=0, pre-check=0"));
...
// -------------------------------------------------------------------
bool isPositive(const String &str) { return str == "1" || str == "true"; }
...
void handleScan(AsyncWebServerRequest *request) {
  ...
  WiFi.scanNetworksAsync(
      [request, response](int networksFound) {
...
void handleSaveEmoncms(AsyncWebServerRequest *request) {
  ...
  config_save_emoncms(isPositive(request->arg(F("enable"))), request->arg(F("server")), request->arg(F("path")),
                      request->arg(F("node")), request->arg(F("apikey")), request->arg(F("fingerprint")));

  char tmpStr[200];
  snprintf_P(tmpStr, sizeof(tmpStr), PSTR("Saved: %s %s %s %s %s"), emoncms_server.c_str(), emoncms_path.c_str(),
             emoncms_node.c_str(), emoncms_apikey.c_str(), emoncms_fingerprint.c_str());
  DBUGLN(tmpStr);
...
void handleSaveMqtt(AsyncWebServerRequest *request) {
  ...
  config_save_mqtt(isPositive(request->arg(F("enable"))), request->arg(F("server")), port, request->arg(F("topic")),
                   request->arg(F("prefix")), request->arg(F("user")), request->arg(F("pass")));

  char tmpStr[200];
  snprintf_P(tmpStr, sizeof(tmpStr), PSTR("Saved: %s %d %s %s %s %s"), mqtt_server.c_str(), port, mqtt_topic.c_str(),
             mqtt_feed_prefix.c_str(), mqtt_user.c_str(), mqtt_pass.c_str());
  DBUGLN(tmpStr);
...
void handleSaveTimer(AsyncWebServerRequest *request) {
  ...
  config_save_timer(request->arg(F("timer_start1")).toInt(), request->arg(F("timer_stop1")).toInt(),
                    request->arg(F("timer_start2")).toInt(), request->arg(F("timer_stop2")).toInt(),
                    request->arg(F("standby_start")).toInt(), request->arg(F("standby_stop")).toInt(),

                    request->arg(F("voltage_output")).toInt());

  mqtt_publish("out/timer", String(timer_start1) + " " + String(timer_stop1) + " " + String(timer_start2) + " " +
                                String(timer_stop2) + " " + String(voltage_output));
...
void handleUpdateGet(AsyncWebServerRequest *request) {
  ...
  response->setCode(200);
  response->print(F("<html><form method='POST' action='/update' enctype='multipart/form-data'>"
...
void handleUpdateUpload(AsyncWebServerRequest *request, const String &filename, size_t index, uint8_t *data, size_t len,
                        bool final) {
  if (!index) {
    dumpRequest(request);
    ...
    int command = U_FLASH;
    size_t updateSize = U_FLASH == command ? (ESP.getFreeSketchSpace() - 0x1000) & 0xFFFFF000
...
void onWsEvent(AsyncWebSocket *server, AsyncWebSocketClient *client, AwsEventType type, void *arg, uint8_t *data,
  ...
  if (type == WS_EVT_CONNECT) {
    ...
    if (info->final && info->index == 0 && info->len == len) {
      ...
      DBUGF("ws[%s][%u] %s-message[%u]: ", server->url(), client->id(), (info->opcode == WS_TEXT) ? "text" : "binary",
            len);
    } else {
...
void onEmonTxEvent(AsyncWebSocket *server, AsyncWebSocketClient *client, AwsEventType type, void *arg, uint8_t *data,
                   size_t len) {
  if (type == WS_EVT_DATA) {
...
void web_server_setup() {
  ...
  server.on("/debug", [](AsyncWebServerRequest *request) { handleDebug(request, SerialDebug); });
  debugBuffer.attach(SerialDebug);

  server.on("/emontx", [](AsyncWebServerRequest *request) { handleDebug(request, SerialEmonTx); });
